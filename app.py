...
# ===================== STYLE (force white, premium) =====================
st.markdown(
    """
<style>
/* Force blanc + supprime dark */
...
/* Table nicer */
[data-testid="stDataFrame"]{ border-radius: 14px; overflow: hidden; border: 1px solid rgba(15,23,42,.10); }
...
def amortization(principal: float, annual_rate_pct: float, n: int, freq: str, start: date):
    ...
    if principal <= 0:
        ...
        df = pd.DataFrame({
            ...
            "Capital remboursé (cumul)": [0.0]*n,
        })
        totals = {"interets_totaux": 0.0, "total_paye": 0.0}
        return 0.0, df, totals
    ...
    for i in range(1, n+1):
        ...
        # avoid negative last balance due to rounding
        if i == n:
        ...
        rows.append({
            ...
            "Solde": balance,
            "Intérêts cumulés": cum_int,
            "Capital remboursé (cumul)": cum_principal
        })
...
with col_inputs:
    ...
    st.markdown('<div class="note">✅ Astuce : tu peux laisser des champs à 0 si tu ne les utilises pas (ex: options).</div>', unsafe_allow_html=True)
...
with col_results:
    if not st.session_state.calculated:
        ...
        with r5:
            ...
            """, unsafe_allow_html=True)
        with r6:
            # Placeholder, useful for analysis by date (below)
            st.markdown(f"""
            <div class="card">
              <h3>Analyse</h3>
        ...
        if df.empty:
            ...
            df_display = df.copy()
            for col in ["Paiement", "Intérêt", "Principal", "Solde", "Intérêts cumulés", "Capital remboursé (cumul)"]:
                df_display[col] = df_display[col].map(lambda v: float(v))
